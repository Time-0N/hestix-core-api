...
use crate::infrastructure::oidc::provider::{OidcAdminApi, IdpUser};
...
#[async_trait]
impl OidcAdminApi for ZitadelManagementClient {
    async fn fetch_all_users(&self) -> Result<Vec<IdpUser>, OidcError> {
        // We need to make this method take &mut self, but the trait expects &self
        // For now, let's implement a workaround by cloning the management client
        // In a real implementation, you'd want to refactor this to use interior mutability

        // Convert the tuple format to IdpUser format
        let mut client = ZitadelManagementClient::new(
            self.client.clone(),
            self.api_url.clone(),
            &serde_json::to_string(&self.service_key).map_err(|e| OidcError::Provider(e.to_string()))?
        ).map_err(|e| OidcError::Provider(e.to_string()))?;

        let users = client.list_all_users().await.map_err(|e| OidcError::Provider(e.to_string()))?;

        Ok(users.into_iter().map(|(_, subject, username, email)| IdpUser {
            idp_subject: subject,
